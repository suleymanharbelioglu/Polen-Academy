...
function toParentEmailLocalPart(firstName: string, lastName: string): string {
 ...
 */
export const createParent = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  if (!parentName || !parentSurname) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "parentName and parentSurname are required."
    );
  }
  ...
  await db.collection("Users").doc(uid).set({
    uid,
    parentName: (parentName as string).trim(),
    parentSurname: (parentSurname as string).trim(),
    email,
